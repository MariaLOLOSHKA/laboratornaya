...
void PipelineManager::printMenu() const {
    ...
    std::cout << "13. Сохранить\n";
    std::cout << "14. Загрузить\n";
    std::cout << "15. Выход\n";
...
void PipelineManager::connectStations() {
    ...
    std::vector<int> freePipes = findFreePipesByDiameter(diameter);
    
    int pipeId;
    if (!freePipes.empty()) {
        // Используем существующую трубу
        pipeId = freePipes[0];
        std::cout << "Используется существующая труба ID: " << pipeId << std::endl;
    } else {
        // Создаем новую трубу
        std::cout << "Свободных труб диаметром " << diameter << " мм не найдено. Создаем новую трубу.\n";
        Pipe newPipe;
...
void PipelineManager::run() {
    ...
    // Сброс счетчиков при запуске программы
    resetCounters();
    ...
    while (true) {
        ...
        if (!(std::cin >> choice)) {
            std::cout << "Ошибка! Введите число от 1 до 15.\n";
            InputUtils::clearInputBuffer();
            continue;
        }
        InputUtils::clearInputBuffer();
        ...
        switch (choice) {
            ...
            case 15:
                ...
                return;
            default:
                std::cout << "Неверный выбор! Введите число от 1 до 15.\n";
        }
